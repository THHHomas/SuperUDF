...
class weight_generator2(nn.Module):
    def __init__(self, input_dim, out_dim):
        super(weight_generator2, self).__init__()
    ...
    def forward(self, feature1, xyz1, query_xyz,  point_index, normal=None, fps_num=None,
                knn_num=12):
        B, N, _ = xyz1.shape
        # M = query_xyz.shape[1]
        ...
        # temp = 0.002/(distance**2+1e-6)
        # weight = torch.softmax(temp, -1)
        weight = torch.softmax(-distance*100, -1)
        # weight = 1/(distance**2+1e-6)

        # weight = torch.softmax(weight*60, -1)
        weight = weight * 0.5
...
class weight_generator(nn.Module):
    def __init__(self, input_dim, out_dim):
        ...
        self.linear_t = nn.Linear(out_dim*2*12, 12)
    ...
    def forward(self, feature1, xyz1, query_xyz, normal=None, fps_num=None, knn_num=12):
        ...
        position_weight = torch.cat([position_weight, g_feature], -1).reshape(B, N, -1)
        weight = self.linear_t(position_weight)
        # scale = F.tanh(self.linear_r(position_weight))*0.5+1
        position_weight = F.softmax(weight, -1)
        # position_weight = position_weight/(position_weight.max(-1, keepdim=True)[0])
