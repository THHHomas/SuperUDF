...
class bsp_network(nn.Module):
    ...
    def mean_shift_error(self, feature, clean_point):
        ...
        shift_2fold_err = self.local_2fold_supervision(feature, clean_point, k_clean)
        shift_3fold_err = self.local_3fold_supervision(feature, clean_point, k_clean)

        error = torch.stack([shift_plane_err, shift_2fold_err, shift_3fold_err]).reshape(3, B * N)
        min_error, action = error.min(0)
        # # plane only
        # action = action*0
        # min_error = shift_plane_err
        # 2-fold only
        # action = torch.ones_like(action) * 1
        # min_error = shift_2fold_err
        # 3-fold only
        # action = torch.ones_like(action)*2
        # min_error = shift_3fold_err
        # # plane + 3fold
        # error = torch.stack([shift_plane_err,  shift_3fold_err]).reshape(2, B * N)
        # min_error, action = error.min(0)
        # action = action*2
        # # plane + 2fold
        # error = torch.stack([shift_plane_err, shift_2fold_err]).reshape(2, B * N)
        # min_error, action = error.min(0)
        # # 3fold + 2fold
        # error = torch.stack([shift_2fold_err, shift_3fold_err]).reshape(2, B * N)
        # min_error, action = error.min(0)
        # action = action + 1
        # L = action[:,:,0]*shift_plane_err + action[:,:,1]*shift_2fold_err + action[:,:,2]*shift_sphere_err
        return min_error, action.reshape(B, N)

    def cal_shift(self, feature, clean_point, detect_point, action, error, knn_num=16, training=True):
        # action = self.action(feature)
        # action = action.max(-1)[1]

        _, shift_sphere = self.cal_3fold_shift(feature, clean_point, detect_point, knn_num)
        _, shift_2fold = self.cal_2fold_shift(feature, clean_point, detect_point, knn_num)
        _, shift_plane = self.cal_plane_shift(feature, clean_point, detect_point, knn_num)
        B, M, K,  _ = shift_plane.shape
        # L = action[:,:,0]*shift_plane_err + action[:,:,1]*shift_2fold_err + action[:,:,2]*shift_3fold_err
        shift_merge = torch.stack([shift_plane, shift_2fold, shift_sphere]).reshape(3, B*M*K, -1)
        point_index = knn_point(knn_num, clean_point, detect_point)
        # shuffle_index = torch.randperm(knn_num)
        # point_index = point_index[:,:,shuffle_index]
        g_action = index_points(action.unsqueeze(2), point_index)[:,:,:,0].reshape(-1)
        shift_chose = shift_merge[g_action, torch.arange(B*M*K)].reshape(B, M, K, -1)

        weight = self.weight_generator(feature, clean_point, detect_point, error, shift_chose,point_index,
                                                    knn_num=knn_num)  # * scale.unsqueeze(2)
        # _, shift_chose = self.cal_2fold_shift(feature, clean_point, detect_point)
        shift = (shift_chose * weight.unsqueeze(3)).sum(-2)
